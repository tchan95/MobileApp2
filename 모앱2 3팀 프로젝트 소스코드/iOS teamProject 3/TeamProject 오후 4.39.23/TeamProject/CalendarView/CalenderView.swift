...
class CalenderView: UIView, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, MonthViewDelegate {
    ...
    var clickDelegate : CalendarViewCellDelegate?
    ...
    // select calendar cell event
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let cell=collectionView.cellForItem(at: indexPath)
        cell?.backgroundColor=Colors.darkRed
        let lbl = cell?.subviews[1] as! UILabel
        lbl.textColor=UIColor.white
        self.selectedDay = Int(lbl.text!)!
        let date = String(format:"%d %@ %@", currentYear, DATE.monthsArr[currentMonthIndex-1], lbl.text!)
        print(date)
        clickDelegate?.onClickCell(with: "TimeListTheme", cell)
    }
